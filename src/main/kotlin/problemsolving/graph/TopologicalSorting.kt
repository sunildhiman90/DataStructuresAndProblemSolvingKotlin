...
fun topologicalSorting(graph: Array<IntArray>): List<Int> {
...
fun helperTopo(
    ...
    for (i in graph[ind].indices) {
        ...
        if (!visited[currentNeighbor]) {
            ...
            helperTopo(currentNeighbor, graph, visited, stack)
        }
    }
...
fun main() {
    ...
    println(topologicalSorting(graph2))
 ...
 * EXPLANATION:-
 *
 * When we are scheduling jobs or tasks, they may have dependencies. For example, before we finish task a,
 * we have to finish b first. In this case, given a set of tasks and their dependencies, how shall we arrange our schedules? There comes an interesting graph algorithm: Topological Sort.
 *
